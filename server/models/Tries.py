...
class CompressedTrie:
    ...
    def update_frequency(self, word: str, k: int) -> bool:
        """
        Set the frequency of an existing word to k.
        Updates the terminal node and refreshes top-k heaps along the entire path.
        Returns True if the word was found and updated, False otherwise.
        """
        if not self.search(word):
            self.insert(word, freq=k)  # if word doesn't exist, insert it with the given frequency
            return False

        self._update_freq_along_path(self.root, word, word, k)
        return True

    def _update_freq_along_path(
        self, node: TrieNode, remaining: str, full_word: str, new_freq: int
    ):
        # replace this word's entry in the current node's top-k
        self._update_topk(node, full_word, new_freq)
        ...
        if remaining.startswith(label):
            self._update_freq_along_path(child, remaining[len(label):], full_word, new_freq)
        else:
            # prefix lands mid-edge — terminal is this child node
            child.frequency = new_freq
            self._update_topk(child, full_word, new_freq)
